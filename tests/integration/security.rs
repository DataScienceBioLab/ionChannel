...
async fn keyboard_rejected_when_only_pointer_authorized() {
    ...
    let event = InputEvent::keyboard_keycode(30, ion_core::event::KeyState::Pressed);
...
async fn touch_rejected_when_not_authorized() {
    ...
    let event = InputEvent::touch_down(0, 0, 100.0, 100.0);
    let result = session.send_event(event).await;
...
async fn all_authorized_devices_work() {
    ...
    let kbd_event = InputEvent::keyboard_keycode(30, ion_core::event::KeyState::Pressed);
    session.send_event(kbd_event).await.expect("keyboard should work");
    let _ = rx.recv().await;

    // Pointer should work
    let ptr_event = InputEvent::pointer_motion(10.0, 5.0);
    session.send_event(ptr_event).await.expect("pointer should work");
    let _ = rx.recv().await;

    // Touch should work
    let touch_event = InputEvent::touch_down(0, 0, 100.0, 100.0);
    session.send_event(touch_event).await.expect("touch should work");
    let _ = rx.recv().await;
...
async fn empty_device_selection() {
    ...
    let kbd = InputEvent::keyboard_keycode(30, ion_core::event::KeyState::Pressed);
    assert!(session.send_event(kbd).await.is_err());

    let ptr = InputEvent::pointer_motion(10.0, 5.0);
    assert!(session.send_event(ptr).await.is_err());

    let touch = InputEvent::touch_down(0, 0, 100.0, 100.0);
