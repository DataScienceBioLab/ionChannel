...
pub async fn get_rustdesk_id(backend: &LibvirtBackend, vm_id: &str) -> Result<String> {
    let result = backend
        .exec_command(vm_id, vec!["rustdesk".to_string(), "--get-id".to_string()])
        .await?;
...
pub async fn deploy_ionchannel(backend: &LibvirtBackend, vm_id: &str) -> Result<()> {
    ...
    // Copy binaries to VM
    let binaries = vec!["target/release/ion-portal", "target/release/ion-compositor"];
...
pub async fn verify_components_running(backend: &LibvirtBackend, vm_id: &str) -> Result<()> {
    ...
    let portal_check = backend
        .exec_command(
            vm_id,
            vec![
                "pgrep".to_string(),
                "-f".to_string(),
                "ion-portal".to_string(),
            ],
        )
        .await?;
