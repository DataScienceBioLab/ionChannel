...
async fn can_send_events_when_active() {
    ...
    session
        .send_event(event)
        .await
        .expect("send_event should succeed");
...
/// Test: App ID is preserved
#[tokio::test]
async fn app_id_preserved() {
    let (tx, _rx) = mpsc::channel(16);
    let session = SessionHandle::new(
        SessionId::new("/test/session/7"),
        "my.cool.app".to_string(),
        tx,
    );

    assert_eq!(session.app_id().await, "my.cool.app");
}

/// Test: Session uptime increases
#[tokio::test]
async fn session_uptime_increases() {
    let (tx, _rx) = mpsc::channel(16);
    let session = SessionHandle::new(
        SessionId::new("/test/session/8"),
        "test-app".to_string(),
        tx,
    );

    let uptime1 = session.uptime().await;
    tokio::time::sleep(std::time::Duration::from_millis(10)).await;
    let uptime2 = session.uptime().await;
