...
async fn build_on_vm(ip: &str, username: &str) -> Result<()> {
    ...
    ssh::execute_remote(ip, username, "cd ~/Development/syntheticChemistry/ionChannel && cargo build --release").await?;
...
async fn deploy_portal(ip: &str, username: &str) -> Result<()> {
    ...
    ssh::execute_remote(ip, username, "cd ~/Development/syntheticChemistry/ionChannel && sudo ./scripts/deploy-to-system.sh").await?;
