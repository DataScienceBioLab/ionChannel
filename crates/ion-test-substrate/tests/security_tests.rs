...
async fn security_session_isolation() {
    ...
    portal.create_session(session_a.to_string(), "app.a".to_string()).await.unwrap();
    ...
    portal.create_session(session_b.to_string(), "app.b".to_string()).await.unwrap();
    ...
    assert!(portal.notify_keyboard_keycode(session_a, 30, KeyState::Pressed).await.is_ok());
    assert!(portal.notify_pointer_motion(session_a, 1.0, 1.0).await.is_err());
...
async fn security_no_cross_session_access() {
    ...
    assert!(portal.notify_pointer_motion(session_b, 1.0, 1.0).await.is_err());
    ...
    assert!(portal.notify_pointer_motion(session_a, 1.0, 1.0).await.is_ok());
}
...
async fn security_device_authorization_strict() {
    ...
    assert!(portal.notify_keyboard_keycode(session_id, 30, KeyState::Pressed).await.is_ok());
    assert!(portal.notify_keyboard_keysym(session_id, 0x61, KeyState::Pressed).await.is_ok());
    
    // Pointer should be rejected
    assert!(portal.notify_pointer_motion(session_id, 1.0, 1.0).await.is_err());
    assert!(portal.notify_pointer_motion_absolute(session_id, 0, 100.0, 100.0).await.is_err());
    assert!(portal.notify_pointer_button(session_id, 1, ButtonState::Pressed).await.is_err());
    assert!(portal.notify_pointer_axis(session_id, 0.0, -10.0).await.is_err());
    
    // Touch should be rejected
    assert!(portal.notify_touch_down(session_id, 0, 0, 50.0, 50.0).await.is_err());
}
...
async fn security_state_machine_integrity() {
    ...
    assert!(portal.notify_pointer_motion(session_id, 1.0, 1.0).await.is_ok());
...
async fn security_input_only_mode() {
    ...
    assert!(!response.capture_available, "Capture should NOT be available in InputOnly");
    assert!(response.input_available, "Input should be available in InputOnly");
...
async fn security_view_only_mode() {
    ...
    assert!(response.capture_available, "Capture should be available in ViewOnly");
    assert!(!response.input_available, "Input should NOT be available in ViewOnly");
}
...
async fn security_none_mode() {
    ...
    assert!(!response.capture_available, "Capture should NOT be available in None mode");
    assert!(!response.input_available, "Input should NOT be available in None mode");
}
...
async fn security_max_sessions_enforced() {
    ...
    // Create max sessions
    for i in 0..max {
        let result = portal.create_session(format!("/security/max/{i}"), format!("app.{i}")).await;
        assert!(result.is_ok(), "Should be able to create session {i}");
    }
    
    // Next should fail
    let result = portal.create_session("/security/max/overflow".to_string(), "overflow".to_string()).await;
    assert!(result.is_err(), "Should not be able to exceed max sessions");
    
    // Close one and retry
    portal.close_session("/security/max/0").await.unwrap();
    let result = portal.create_session("/security/max/new".to_string(), "new".to_string()).await;
...
async fn security_input_validation() {
    ...
    let _ = portal.notify_pointer_motion(session_id, -1000.0, -1000.0).await;
    let _ = portal.notify_pointer_motion_absolute(session_id, 0, -100.0, -100.0).await;
    
    // Huge coordinates  
    let _ = portal.notify_pointer_motion(session_id, 1e10, 1e10).await;
    
    // Negative button/key codes
    let _ = portal.notify_pointer_button(session_id, -1, ButtonState::Pressed).await;
    let _ = portal.notify_keyboard_keycode(session_id, -1, KeyState::Pressed).await;
    
    // Extreme slot/stream values for touch
    let _ = portal.notify_touch_down(session_id, u32::MAX, u32::MAX, 0.0, 0.0).await;
