...
async fn e2e_basic_session_lifecycle() {
    ...
    let response = portal.create_session(session_id.to_string(), app_id.clone()).await.unwrap();
    assert_eq!(response.session_id, session_id, "CreateSession should return session_id");
...
async fn e2e_input_event_flow() {
    ...
    portal.create_session(session_id.to_string(), app_id).await.unwrap();
    ...
    // Pointer events
    portal.notify_pointer_motion(session_id, 10.0, 20.0).await.unwrap();
    event_count += 1;
    
    portal.notify_pointer_motion_absolute(session_id, 0, 100.0, 200.0).await.unwrap();
    event_count += 1;
    
    portal.notify_pointer_button(session_id, 1, ButtonState::Pressed).await.unwrap();
    event_count += 1;
    
    portal.notify_pointer_button(session_id, 1, ButtonState::Released).await.unwrap();
    event_count += 1;
    
    portal.notify_pointer_axis(session_id, 0.0, -10.0).await.unwrap();
    event_count += 1;
    ...
    portal.notify_keyboard_keysym(session_id, 0x61, KeyState::Pressed).await.unwrap();
    event_count += 1;
    
    // Touch events
    portal.notify_touch_down(session_id, 0, 0, 50.0, 50.0).await.unwrap();
    event_count += 1;
    
    portal.notify_touch_motion(session_id, 0, 0, 60.0, 60.0).await.unwrap();
    event_count += 1;
...
async fn e2e_multi_session() {
    ...
    for (path, app_id) in &sessions {
        portal.create_session(path.to_string(), app_id.to_string()).await.unwrap();
    }
    ...
    for (i, (path, _)) in sessions.iter().enumerate() {
        portal.notify_pointer_motion(path, (i + 1) as f64 * 10.0, (i + 1) as f64 * 10.0)
            .await
            .unwrap();
    }
    ...
    println!("✅ E2E: Multi-session ({} sessions) completed successfully", sessions.len());
}
...
async fn e2e_error_handling() {
    ...
    assert!(result.is_err(), "SelectDevices on non-existent session should fail");
    ...
    let result = portal.notify_pointer_motion("/nonexistent/session", 1.0, 1.0).await;
    ...
    let result = portal.create_session(session_id.to_string(), "test.app".to_string()).await;
...
async fn e2e_stress_test() {
    ...
    portal.create_session(session_id.to_string(), "stress.test".to_string()).await.unwrap();
    ...
    for i in 0..event_count {
        portal.notify_pointer_motion(session_id, (i % 100) as f64, (i % 50) as f64)
            .await
            .unwrap();
    }
